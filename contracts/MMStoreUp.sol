...
contract MMStore is IMMStore,OwnableUpgradeable,UUPSUpgradeable{
    ...
    modifier onlyConfig(){
        if(msg.sender != acbConfig) revert NotConfig();
        _;
    }
    ...
    function initialize(address _acbConfig)external initializer{
        ...
        //users.push(
            //User(ACCONFIG(acbConfig).top(),address(0),block.timestamp)
       // );
    }
    ...
    function buyMachine(uint256 amount,uint256 bType)external {
        ...
        //distribute usdt
        IERC20Upgradeable(_usdt()).transfer(ACCONFIG(acbConfig).techAddress(), amount / 2 * 20 / 1000); //2% to tech fee
        IERC20Upgradeable(_usdt()).transfer(ACCONFIG(acbConfig).marketAddress(), amount / 2 * 20 / 1000); //2% to market fee
    ...
    function claim()external {
        ...
        for(uint256 i = 0; i < machines.length; ++i){
            ...
            if(block.timestamp - machine.lastTime <= ACCONFIG(acbConfig).claimInterval()) revert TimeNotYet();
    ...
    function claimDynamic(uint256 refAmount,uint256 team,uint256 sameLevel,uint256 share,bytes memory signature)external {
        ...
        if(ACCONFIG(acbConfig).operator() != recoverAddress) revert NotOp();
    ...
    function getReserves() public view returns (uint112 _reserve0, uint112 _reserve1, uint32 _blockTimestampLast) {
        (  _reserve0,   _reserve1,) = IACBPair_(_acbPair()).getReserves();
    ...
    function _minePool()internal view returns(address){
        return ACCONFIG(acbConfig).minePool();
    }

    function _usdt()internal view returns(address){
        return ACCONFIG(acbConfig).usdt();
    }

    function _acb()internal view returns(address){
        return ACCONFIG(acbConfig).acb();
    }
    ...
    function _acbPair()internal view returns(address){
        return ACCONFIG(acbConfig).acbPair();
    }

    function _machineExpireTime()internal view returns(uint256){
        return ACCONFIG(acbConfig).machineExpireTime();
    }
    ...
    function machineReward(address user)external view returns(MachineReward memory rewardInfo){
        ...
        for(uint256 i = 0; i < machines.length; ++i){
            ...
            if(block.timestamp - machine.lastTime <= ACCONFIG(acbConfig).claimInterval()){
                canClaim = false;
            }
    ...
    function userMachineArr(address user)external view returns(MachineResponse[] memory list){
        ...
        for(uint256 i = 0; i < idList.length; i++){
            ...
            uint256 countdown = mm.createTime + ACCONFIG(acbConfig).machineExpireTime() > block.timestamp ? 
                                            mm.createTime + ACCONFIG(acbConfig).machineExpireTime() - block.timestamp : 0;
